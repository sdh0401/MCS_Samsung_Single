		  ...
		  )
{
	...
	pThread->m_state = ThreadState_t::ThreadStateWaiting;
	...
	while( TRUE )
	{
		...
		if( lastType == ThreadType_t::ThreadTypeHomogeneous ||
			lastType == ThreadType_t::ThreadTypeSpecialized ||
			lastType == ThreadType_t::ThreadTypeNotDefined )
		{
		...
		if( pThread->m_type == ThreadType_t::ThreadTypeIntervalDriven )
			Sleep(pThread->m_dwIdle);
	}
	pThread->m_mutex.Lock();
	pThread->m_state = ThreadState_t::ThreadStateDown;
...
{
    ASSERT(lpvData && m_type == ThreadType_t::ThreadTypeHomogeneous);

	if( m_type != ThreadType_t::ThreadTypeHomogeneous )
	{
		cerr << "Warning CThread::OnTask:\n\tOnTask(LPVOID) called for a non-homogeneous thread!\n";
		return FALSE;
	}

	((CTask *)lpvData)->SetTaskStatus(TaskStatus_t::TaskStatusBeingProcessed);
	BOOL bReturn = ((CTask *)lpvData)->Task();
	((CTask *)lpvData)->SetTaskStatus(TaskStatus_t::TaskStatusCompleted);
...
CThread::OnTask()
{
	ASSERT(m_type == ThreadType_t::ThreadTypeIntervalDriven);
	if( m_type != ThreadType_t::ThreadTypeIntervalDriven )
	{
			   ...
			   )
{
	...
	ASSERT(m_type == ThreadType_t::ThreadTypeHomogeneous ||
	       m_type == ThreadType_t::ThreadTypeNotDefined );
	...
	try 
	{
			...
			m_type != ThreadType_t::ThreadTypeNotDefined    )
		{
			...
			m_state = ThreadState_t::ThreadStateFault;
			cerr << "Warning: invalid call to CEvent::Event(CTask *), thread type is not specialized\n";

			return FALSE;
		}

		m_type = ThreadType_t::ThreadTypeHomogeneous;
		m_mutex.Unlock();
		...
		pvTask->SetTaskStatus(TaskStatus_t::TaskStatusWaitingOnQueue);
		m_event.Set();
			   ...
			   )
{
	...
	m_mutex.Lock();
	ASSERT( m_type == ThreadType_t::ThreadTypeSpecialized ||
		    m_type == ThreadType_t::ThreadTypeNotDefined );
	try 
	{
		if( FromSameThread() )
	...
	if( m_type != ThreadType_t::ThreadTypeSpecialized &&
		m_type != ThreadType_t::ThreadTypeNotDefined )
	{
		m_dwObjectCondition |= ILLEGAL_USE_OF_EVENT;
		m_dwObjectCondition |= EVENT_AND_TYPE_DONT_MATCH;
	...
	m_type = ThreadType_t::ThreadTypeSpecialized;
...
CThread::KernelProcess()
{
	...
	m_mutex.Lock();
	m_state = ThreadState_t::ThreadStateBusy;
	if( !m_bRunning )
	{
		m_state = ThreadState_t::ThreadStateShuttingDown;
		m_mutex.Unlock();
		return FALSE;
	}
	m_mutex.Unlock();
	...
	if( !Empty() )
	{
		...
		m_mutex.Lock();
		m_lpvProcessor = NULL;
		m_state = ThreadState_t::ThreadStateWaiting;
	}
	else {
		if( !OnTask() )
		{
...
#endif
,m_dwId(0L)
,m_state(ThreadState_t::ThreadStateDown)
,m_dwIdle(100)
,m_lppvQueue(NULL)
,m_lpvProcessor(NULL)
...
CThread::Stop()
{
	try 
	{
		...
		for( int i=0; i<ticks; i++ )
		{
			...
			m_mutex.Lock();
			if( m_state == ThreadState_t::ThreadStateDown )
			{
				m_mutex.Unlock();
				return TRUE;
			}
...
CThread::AtCapacity()
{
	m_mutex.Lock();
		if( ((m_dwObjectCondition & STACK_OVERFLOW ||
			  m_dwObjectCondition & STACK_FULL ) &&
			  m_state == ThreadState_t::ThreadStateBusy) || !m_bRunning)
		{
			m_mutex.Unlock();
			return TRUE;
		}
