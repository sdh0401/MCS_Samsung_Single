...
#error "Platform not supported!"
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (frameNum == 0)
      et = firstEntry;
    this->OnCallstackEntryOnlyFunc(et, csEntry);
    
    if (s.AddrReturn.Offset == 0)
    {
      this->OnCallstackEntryOnlyFunc(lastEntry, csEntry);
...
void StackWalker::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName, ULONGLONG fileVersion)
{
  ...
  else
  {
    ...
    _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s:%s (%p), size: %d (result: %d), SymType: '%s', PDB: '%s', fileVersion: %d.%d.%d.%d\n", 
        img, mod, (LPVOID) baseAddr, (long)size, (long)result, symType, pdbName, (long)v1, (long)v2, (long)v3, (long)v4);
  }
  //OnOutput(buffer);
}
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
      ...
      _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s (%d): %s\n", entry.lineFileName, (long)entry.lineNumber, entry.name);
    OnOutput(buffer);
  }
}

void StackWalker::OnCallstackEntryOnlyFunc(CallstackEntryType eType, CallstackEntry& entry)
{
    CHAR buffer[STACKWALK_MAX_NAMELEN];
    if ((eType != lastEntry) && (entry.offset != 0))
    {
        ...
        if (entry.lineFileName[0] == 0)
        {
            ...
            if (entry.moduleName[0] == 0)
                strcpy_s(entry.moduleName, "(module-name not available)");
            //_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%p (%s): %s: %s\n", (LPVOID)entry.offset, entry.moduleName, entry.lineFileName, entry.name);
            _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "[PWR] Trace - %s\n", entry.name);
        }
        else
        {
...
{
  CHAR buffer[STACKWALK_MAX_NAMELEN];
  _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "[PWR] SymInit: Symbol-SearchPath: '%s', symOptions: %d, UserName: '%s'\n", szSearchPath, (long)symOptions, szUserName);
  //OnOutput(buffer);
  ...
  if (GetVersionExA(&ver) != FALSE)
  {
    ...
    if (VerifyVersionInfo(&osvi, VER_MAJORVERSION | VER_MINORVERSION, dwlConditionMask) != FALSE)
    {
        _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "[PWR] OS-Version: %d.%d.%d\n",
            (long)osvi.dwMajorVersion, (long)osvi.dwMinorVersion, (long)osvi.dwBuildNumber);
        //OnOutput(buffer);
    }
#endif
}
