...
public:
	...
	BOOL Wait(int timeoutSeconds)
	{
		...
		if( Status() != TaskStatus_t::TaskStatusCompleted &&
			timeoutSeconds > 0 )
		{
			Sleep(100);
			timeoutSeconds = timeoutSeconds - 100;
		}
		if( Status() == TaskStatus_t::TaskStatusCompleted ) return TRUE;
		return FALSE;
	}
	...
	CTask(){m_state = TaskStatus_t::TaskStatusNotSubmitted; memset(&m_dwThread,0,sizeof(ThreadId_t)); }
	~CTask(){}
	virtual BOOL Task()=0;
};
...
public:
	...
	void		SetThreadType(ThreadType_t typ = ThreadType_t::ThreadTypeNotDefined,DWORD dwIdle=100);
